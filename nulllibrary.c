...
void
astrxcpy(unsigned char* dst, char* src, size_t size)
{
    int len = strlen(src);
    if(size < len)
        len = size;
...
{
    trace = fopen("/tmp/trace", "a");
    if (trace) {
        fprintf(trace,"Initialize()\n");
    }
...
{
    if (trace) {
        fprintf(trace,"GetSlotList(%d,,%ld)\n",token_present,*count);
    }
    if (slot_list) {
        slot_list[0] = 0;
    }
...
OpenSession(CK_SLOT_ID slot_id, CK_FLAGS flags, void *application, CK_NOTIFY notify, CK_SESSION_HANDLE *session)
{
    ...
    if (trace) {
        fprintf(trace,"OpenSession(%ld,%ld,,,)\n",slot_id,flags);
    }
...
Login(CK_SESSION_HANDLE session, unsigned long user_type, unsigned char *pin, unsigned long pin_len)
{
    struct session_struct* sessionImpl = (struct session_struct*)session;
    if (trace) {
        fprintf(trace,"Login(,%ld,%s,%ld)\n",user_type,pin,pin_len);
    }
        ...
        for (i=0; i<count; i++) {
            switch (templ[i].type) {
                case CKA_ID:
                    ...
                    break;
                case CKA_KEY_TYPE:
                    fprintf(trace, "{CKA_KEY_TYPE,,%ld},", templ[i].ulValueLen);
                    break;
                case CKA_PRIME:
                    fprintf(trace, "{CKA_PRIME,,%ld},", templ[i].ulValueLen);
                    break;
                case CKA_SUBPRIME:
                    fprintf(trace, "{CKA_SUBPRIME,,%ld},", templ[i].ulValueLen);
                    break;
                case CKA_BASE:
                    fprintf(trace, "{CKA_BASE,,%ld},", templ[i].ulValueLen);
                    break;
                case CKA_VALUE:
                    fprintf(trace, "{CKA_VALUE,,%ld},", templ[i].ulValueLen);
                    break;
                default:
                    fprintf(trace, "{%ld,,%ld},", templ[i].type, templ[i].ulValueLen);
                    break;
            }
        }
        fprintf(trace, "],%ld)\n", count);
...
FindObjectsInit(CK_SESSION_HANDLE session, CK_ATTRIBUTE* templ, unsigned long count)
{
    int i, j;
    if (trace) {
        fprintf(trace, "FindObjectsInit(,,[");
        for (i = 0; i < count; i++) {
            switch (templ[i].type) {
                case CKA_CLASS:
                    fprintf(trace,"{CKA_CLASS,[");
                    for(j=0; j<templ[i].ulValueLen; j++) {
                        fprintf(trace,"%02x",((unsigned char*)templ[i].pValue)[j]);
                    }
                    fprintf(trace,"],%ld},",templ[i].ulValueLen);
                    break;
                case CKA_ID:
                    fprintf(trace,"{CKA_ID,[");
                    for(j=0; j<templ[i].ulValueLen; j++) {
                        fprintf(trace,"%02x",((unsigned char*)templ[i].pValue)[j]);
                    }
                    fprintf(trace,"],%ld},",templ[i].ulValueLen);
...
FindObjects(CK_SESSION_HANDLE session, CK_OBJECT_HANDLE* object, unsigned long max_object_count, unsigned long *object_count)
{
    int i;
    if (trace) {
        fprintf(trace, "FindObjects(,%p,%ld,)\n",object,max_object_count);
    }
    *object_count = 1;
...
{
    if (trace) {
        fprintf(trace, "FindObjectsFinal()\n");
    }
    return CKR_OK;
}
...
SignInit(CK_SESSION_HANDLE session, CK_MECHANISM* mechanism_ptr, CK_OBJECT_HANDLE key)
{
    int j;
    if (trace) {
        fprintf(trace, "SignInit(,{%ld,", mechanism_ptr->mechanism);
        for (j = 0; j < mechanism_ptr->ulParameterLen; j++) {
            fprintf(trace, "%02x", ((unsigned char*) mechanism_ptr->pParameter)[j]);
        }
        fprintf(trace, ",%ld},)\n",mechanism_ptr->ulParameterLen);
    }
    return CKR_OK;
}
...
{
    if (trace) {
        fprintf(trace, "GenerateRandom(,,%ld)\n",random_len);
    }
